...
def loadExcel():
    ...
    wb = openpyxl.load_workbook(r'InterfaceUpdate\CDP_Neighbor_Info.xlsx')
    ws = wb.active
    ...
    for i in range(5, ws.max_row + 1):
        ...
        for j in range(1, ws.max_column + 1):
            ...
            if cellValue == None:
                interface = ws.cell(row = i, column = j + 2).value
                uplink = ws.cell(row = i, column = j + 3).value
                excelData[-1].extend([interface, uplink])
                break
            else:
...
def commandExecute(session, device, delimeter='switch'):
    ...
    for i in range(0, len(device[2:]), 2):

        command = 'interface ' + device[2:][i]
        session.sendline(command)
        session.expect(['#', wexpect.EOF])

        print(session.before)
        if delimeter == 'switch':
            command = 'desc UPLINK ' + device[2:][i + 1]
        elif delimeter == 'AP':
            command = 'desc ' + device[2:][i + 1]
...
if __name__ == '__main__':
    ...
    for elem in devices:
        ...
        # Switch & Router : no need to set delimeter
        # AP : delimeter should be set to AP
        commandExecute(session, elem, delimeter='AP')
        session.close()